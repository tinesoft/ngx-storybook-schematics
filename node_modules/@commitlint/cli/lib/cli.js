...
function main(options) {
	return new Promise(function ($return, $error) {
		...
		return Promise.resolve(fromStdin ? stdin() : read(range, { cwd: flags.cwd })).then(function ($await_1) {
			try {
				input = $await_1;
				messages = (Array.isArray(input) ? input : [input]).filter(message => typeof message === 'string').filter(Boolean);
				...
				return Promise.resolve(load(getSeed(flags), loadOpts)).then(function ($await_2) {
					try {
						...
						return $return(Promise.all(messages.map(message => new Promise(function ($return, $error) {
